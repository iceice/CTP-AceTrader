...
StopOrderTable::StopOrderTable()
{
	...
	QStringList header;
	header << "编号" << "合约代码" << "买卖" << "开平" << "委托数" << "止损价" << "报单状态" << "报单时间" << "报单日期";
	...
	this->setColumnWidth(6, 120);
	this->setColumnWidth(7, 96);
...
void StopOrderTable::focusOutEvent(QFocusEvent * event)
{
	this->clearSelection();
}

void StopOrderTable::update() {
	this->clearContents();
	...
	for (auto iter = allOrder.rbegin(); iter != allOrder.rend(); iter++) {
		...
		if (i->OrderStatus == '0')
			...
			status = "已触发";
		seventh->setText(status);
		seventh->setTextAlignment(Qt::AlignCenter);
		eighth->setText(i->InsertTime);
		eighth->setTextAlignment(Qt::AlignCenter);
		ninth->setText(i->InsertDate);
